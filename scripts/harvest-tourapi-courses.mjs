...
async function run(){
    ...
    for(const areaCode of AREAS){
      ...
      for await (const j of pagedJson(build,1,10)){
        ...
        for(const it of items){
          out.push({ source:'tourapi', dataset:'course', external_id:String(it.contentid), lang, payload:it, city: it.addr1||null });
        }
  ...
  console.log('tourapi courses saved:', res.count);
}
