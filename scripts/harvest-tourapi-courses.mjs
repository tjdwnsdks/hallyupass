// TourAPI 여행코스(contentTypeId=25) → raw_sources(dataset: 'course')
...
async function fetchPage({ key, areaCode, lang, pageNo }){
  ...
  const url = buildUrl(base, key, {
    ...
    contentTypeId:25, areaCode, numOfRows:100, arrange:'C', pageNo, lang
  });
...
async function run(){
  ...
  for(const lang of langs){
    for(const areaCode of areas){
      console.log(`[FETCH] course area=${areaCode} lang=${lang}`);
      for(let page=1; page<=20; page++){
        const items = await fetchPage({ key, areaCode, lang, pageNo:page });
        if(items.length===0) break;
        for(const it of items){
          await upsertRaw({
            source:'tourapi', dataset:'course', external_id:String(it.contentid),
