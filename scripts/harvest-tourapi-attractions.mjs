...
async function fetchPage(params, attempt = 1) {
  const url = `${BASE}?` + qs({ serviceKey: KEY, _type:'json', MobileOS:'ETC', MobileApp:'HallyuPass', contentTypeId:12, numOfRows:100, arrange:'C', ...params });
  try {
    const j = await fetchJsonSmart(url, { label:'tourapi' });
    const items = j?.response?.body?.items?.item || [];
    return Array.isArray(items) ? items : (items ? [items] : []);
  } catch (e) {
    if (e.name === 'OpenAPIError' && (e.code === '22' || /LIMITED_/.test(e.authMsg||'')) && attempt <= 3) {
    ...
    for (const areaCode of AREAS) {
      ...
      for (let pageNo = 1; pageNo <= 30; pageNo++) {
        ...
        for (const it of items) {
          ...
          out.push({ source:'tourapi', dataset:'attractions', external_id:ext, lang, payload:it, city:it.addr1 ?? null });
        }
