...
const ENDPOINT = SVC.endsWith('2') ? 'searchFestival2' : 'searchFestival'; // KorService2 사용 시 2 서픽스
...
async function run(){
  ...
  for(const lang of LANGS){
    for(const areaCode of AREAS){
      console.log(`[FETCH] festivals area=${areaCode} lang=${lang} ${eventStartDate}-${eventEndDate} svc=${SVC}`);
