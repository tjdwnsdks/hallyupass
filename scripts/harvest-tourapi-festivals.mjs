    ...
    for(const areaCode of AREAS){
      ...
      for await (const j of pagedJson(build,1,10)){
        ...
        for(const it of items){
          out.push({ source:'tourapi', dataset:'festival', external_id:String(it.contentid), lang,
                     payload:it, event_start:toDate(it.eventstartdate), event_end:toDate(it.eventenddate), city: it.addr1||null });
        }
