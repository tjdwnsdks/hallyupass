import { qs, fetchJson, todayYmd, plusDaysYmd, sleep } from './lib/util.mjs';
...
const FROM = todayYmd(); const TO = plusDaysYmd(AHEAD);
...
async function fetchAreaLang(area, lang){
  ...
  while(true){
    const url = `${base}?` + qs({ ...baseParams(), eventStartDate:FROM, eventEndDate:TO, areaCode:area, pageNo:page });
    let j;
    try{
